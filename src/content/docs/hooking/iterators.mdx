---
title: Hooking Iterators
description: Learn how iterator methods work and how to hook them — IEnumerable, IEnumerable<T>, IEnumerator, IEnumerator<T>
sidebar:
  order: 1
---
...
[Iterator methods](<https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/concepts/iterators>), which include methods returning a value which is
...
[`IEnumerator`](https://learn.microsoft.com/en-us/dotnet/api/system.collections.ienumerator), or
[`IEnumerator<T>`](https://learn.microsoft.com/en-us/dotnet/api/system.collections.generic.ienumerator-1),
are quite special, and this article discusses hooking them.

Take for example a method returning `IEnumerator<string>`, although it could be any of the above, and it doesn't significantly matter whether or not the method is static, but here it is static:
...
The C# compiler transforms iterators such as this into methods which return a state machine:
...
Because of how small methods that construct and return an enumerator class usually are, the JIT compiler uses an optimization where it essentially copies the small method to a calling method. This happens if a method which calls the enumerator getter method gets compiled before you hook the enumerator getter method.
...
static class MyHooks
{
    ...
    static IEnumerator<string> Wrapper_GetMessages(IEnumerator<string> original)
    {
        ...
        // Enumerate the whole original enumerator object to completion
        while (original.MoveNext())
        {
            // On each MoveNext call of the original, we return its return value.
            // Or, we could also return a different value if we wanted to.
...
[Iterators](<https://learn.microsoft.com/en-us/dotnet/csharp/programming-guide/concepts/iterators>) can be hooked multiple ways, but hooking the MoveNext method is recommended by MonoDetour.

:::note
The next major version `0.8` (current is `0.7.x`) of MonoDetour.HookGen will change the hook helper method generation for `MoveNext`, so that e.g. `Md.Class.Method.PrefixMoveNext` becomes `Md.Class.Method.MoveNext.Prefix`. This is a compile-time breaking change; you can safely keep using `0.7.x` at runtime.

See [MonoDetour.HookGen: use static extension methods #17](<https://github.com/MonoDetour/MonoDetour/pull/17>) for more info.
:::
