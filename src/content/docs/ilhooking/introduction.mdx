...
The following example is written using the `ILWeaver` API from MonoDetour. Note that you don't need to use it, although it's one of, if not the most user friendly IL manipulation API there is. Right now you aren't expected to know how the `ILWeaver` API works, that will be explained in later articles in further detail.

```cs
// Tip: Always describe why your ILHook exists
// and why it does what it does.

// We need the return value of ReturnHelloWorld to be
// "ILHooked" rather than "Hello, World!" because of
// <insert reason here>
...
static void ILHook_ReturnHelloWorld(ILManipulationInfo info)
{
    ...
    w.MatchRelaxed(x => x.MatchLdstr("Hello, World!") && w.SetCurrentTo(x))
        .ThrowIfFailure()
        .ReplaceCurrentOperand("ILHooked!");
}
```
...
<LinkCard
  ...
  href="/ilhooking/best-practices"
  description="Learn why we do instruction matching with 'MatchRelaxed' along other things"
/>
